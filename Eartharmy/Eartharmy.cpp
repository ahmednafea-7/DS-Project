...
bool Eartharmy::AddUnit(Unitarmy* unit)
{
	...
	else if(unit->GetType() == "SU")
		return SU_list.enqueue(dynamic_cast<SaverUnit*>(unit));
...
void Eartharmy::printEarth()
{
	...
	if (ES_list.peek(es))
		cout << "Infection percentage" << es->GetInfCount() << "/" << ES_list.GetCount() << "=" << ((double(es->GetInfCount())) / double(ES_list.GetCount())) * 100 << "%" << endl; // for testing
	...
	cout << endl;
	cout << SU_list.GetCount() << " SU "; SU_list.print();
	cout << endl << endl;
}
...
LinkedQueue<SaverUnit*>& Eartharmy::GetSUList()
{
	return SU_list;
}
