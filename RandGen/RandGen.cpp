...
void RandGen::setN_Prob(int n, int P,int Inf,int Inf_threshold)
{
	N = n;
	Prob = P;
	Infect_Prob = Inf;
	Infect_Threshold = Inf_threshold;
}
...
void RandGen::generateUnit()
{
	...
	if (A1 < Prob)
	{
		for (int i = 0; i < N; i++) {
			...
			else if(B < ES + ET + EG + HU)
			{
				gameptr->getEartharmy()->AddUnit(new HealUnit(Earth_id++, gameptr->getTimestep(), Health, power/2, Attackcap, gameptr));
			}
			if (gameptr->getEartharmy()->GetESList().GetCount() != 0) {
				if (100 * (float(Earthsoldier::GetInfCount()) / gameptr->getEartharmy()->GetESList().GetCount()) > Infect_Threshold)
				{
					gameptr->getEartharmy()->AddUnit(new SaverUnit(Earth_id++, gameptr->getTimestep(), Health, power, Attackcap, gameptr));
				}
			}
			}
	}
